...
As demais recebem valores neutros (mediana do treino).
O pipeline faz encoding e scaling automaticamente — dados brutos entram.
</p>
...
# ── Medianas do treino ────────────────────────────────────────
MEDIANAS = {
    'Age': 36.0, 'DailyRate': 802.0, 'Education': 3.0,
    'HourlyRate': 66.0, 'JobInvolvement': 3.0,
    'MonthlyRate': 14235.0, 'PercentSalaryHike': 14.0,
    'StockOptionLevel': 1.0, 'TrainingTimesLastYear': 3.0,
    'WorkLifeBalance': 3.0, 'YearsAtCompany': 5.0,
    'YearsSinceLastPromotion': 1.0, 'Gender': 'Male',
    'Department': 'Research & Development',
    'EducationField': 'Life Sciences',
    'JobRole': 'Research Scientist', 'MaritalStatus': 'Married',
}
...
with col1:
    ...
    overtime = st.selectbox(
        "1. OverTime — Faz hora extra?", ["No", "Yes"],
        help="SHAP=0.588 — maior preditor"
    )
    total_working = st.slider(
        "2. TotalWorkingYears — Anos de experiência", 0, 40, 10,
        help="Usado para calcular YearsPerCompany (SHAP=0.387)"
    )
    num_companies = st.slider(
        "   NumCompaniesWorked — Nº empresas anteriores", 0, 9, 2
    )
    business_travel = st.selectbox(
        "3. BusinessTravel — Viagens",
        ["Non-Travel", "Travel_Rarely", "Travel_Frequently"],
        index=1, help="SHAP=0.486 (Frequently)"
    )
    env_satisfaction = st.select_slider(
        "4. EnvironmentSatisfaction", options=[1,2,3,4], value=3,
        format_func=lambda x:{1:"Baixa",2:"Média",3:"Alta",4:"Muito Alta"}[x],
        help="SHAP=0.378"
    )
...
with col2:
    ...
    job_satisfaction = st.select_slider(
        "5. JobSatisfaction", options=[1,2,3,4], value=3,
        format_func=lambda x:{1:"Baixa",2:"Média",3:"Alta",4:"Muito Alta"}[x],
        help="SHAP=0.380"
    )
    ...
    distance = st.slider(
        "7. DistanceFromHome (km)", 1, 29, 7, help="SHAP=0.317"
    )
    marital_status = st.selectbox(
        "8. MaritalStatus",
        ["Single", "Married", "Divorced"], index=1,
        help="SHAP=0.356"
    )
    age = st.slider("9. Age", 18, 60, 36, help="SHAP=0.248")
    rel_satisfaction = st.select_slider(
        "10. RelationshipSatisfaction", options=[1,2,3,4], value=3,
        format_func=lambda x:{1:"Baixa",2:"Média",3:"Alta",4:"Muito Alta"}[x],
        help="SHAP=0.248"
    )
...
if predict_btn:
    ...
    promocao_atrasada     = float(MEDIANAS['YearsSinceLastPromotion'] > 3)
    jovem_sem_experiencia = float(age < 30 and total_working < 3)

    # ── Input RAW — pipeline faz o resto ─────────────────────
    dados = pd.DataFrame([{
        'Age':                      float(age),
    ...
    col_r1.metric("Score de Risco",
                  f"{proba*100:.1f}%")
    col_r2.metric("Threshold",
                  f"{THRESHOLD*100:.0f}%")
    ...
    <div style="background:#161b22;border:2px solid {cor};
        ...
        <div style="color:#8b949e;font-size:0.85rem;">
            ...
            →
            {'Attrition previsto' if pred==1
             else 'Sem attrition previsto'}
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if distance > 14:
        fatores.append("⚠️ Distância de casa > 14km (SHAP=0.317)")
    if age < 30:
    ...
    c1.metric("YearsPerCompany",     f"{years_per_company:.2f}")
    c2.metric("IncomePerYear",       f"{income_per_year:.1f}")
    c3.metric("JovemSemExperiencia",
              "Sim" if jovem_sem_experiencia else "Não")

    st.markdown("""
    <p style='color:#8b949e;font-size:0.8rem;margin-top:12px;'>
    Pipeline: ImbPipeline(preprocessor + RUS + LogReg ElasticNet).
